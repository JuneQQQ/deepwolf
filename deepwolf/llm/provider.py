...
deepwolf never hard-codes a vendor. A *provider* is anything that can turn a
...
DeepSeek, Xiaomi MiMo, local servers — exposes. Swap providers by changing one
...
# Known OpenAI-compatible endpoints. Pick one with DEEPWOLF_PROVIDER, or set
# DEEPWOLF_BASE_URL directly for anything else.
...
class LLMConfig:
    ...
    def from_env(cls, env_file: str | os.PathLike[str] | None = ".env") -> LLMConfig:
        ...
        Reads ``DEEPWOLF_API_KEY``, ``DEEPWOLF_MODEL`` and either
        ``DEEPWOLF_BASE_URL`` or ``DEEPWOLF_PROVIDER`` (a key of :data:`PRESETS`).
        A ``.env`` file in the working directory is loaded first if present.
        """
        if env_file is not None:
            _load_dotenv(Path(env_file))

        api_key = os.environ.get("DEEPWOLF_API_KEY", "")
        model = os.environ.get("DEEPWOLF_MODEL", "")
        base_url = os.environ.get("DEEPWOLF_BASE_URL", "")
        provider = os.environ.get("DEEPWOLF_PROVIDER", "")

        if not base_url and provider:
            if provider not in PRESETS:
                raise LLMError(
                    f"unknown DEEPWOLF_PROVIDER {provider!r}; "
                    f"known: {', '.join(sorted(PRESETS))}"
                )
            base_url = PRESETS[provider]
        if not (base_url and api_key and model):
            raise LLMError(
                "missing LLM config: set DEEPWOLF_API_KEY, DEEPWOLF_MODEL and "
                "DEEPWOLF_BASE_URL (or DEEPWOLF_PROVIDER). See .env.example."
            )
        return cls(
            base_url=base_url.rstrip("/"),
            api_key=api_key,
            model=model,
            temperature=float(os.environ.get("DEEPWOLF_TEMPERATURE", 0.8)),
            max_tokens=int(os.environ.get("DEEPWOLF_MAX_TOKENS", 600)),
        )
