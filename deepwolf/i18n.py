"""Localisation for deepwolf — English and Simplified Chinese.

This module is a leaf: it imports nothing else from deepwolf, so any layer can
use it freely. A :class:`Translator` turns message keys into formatted text in
the chosen language. Player ids like ``(P0)`` are kept verbatim in every
language; only words are translated.
